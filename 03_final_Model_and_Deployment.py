...
text_input = img_bs64_list[:1][0]
payload_pd = pd.DataFrame([[text_input]],columns=['text'])
tst = payload_pd.iloc[:,0].to_list()[0]
tst_s = base64.b64decode(tst)
tst_s = Image.open(BytesIO(tst_s))
model.encode([tst_s], convert_to_tensor=True, show_progress_bar=False)
